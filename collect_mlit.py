...
国土交通省「不動産情報ライブラリ API」から東京都の成約価格情報を取得し、
中古マンションデータだけを CSV 保存する。
...
def filter_used_condominiums(records: list[dict]) -> list[dict]:
    return [
        record
        for record in records
        if "中古マンション" in str(record.get("Type", ""))
    ]
    ...
    with requests.Session() as session:
        for year in range(START_YEAR, END_YEAR + 1):
            for quarter in range(1, 5):
                ...
                print(f"中古マンション: {len(condominiums):,}件")
                all_records.extend(condominiums)
                time.sleep(REQUEST_INTERVAL_SECONDS)

    if not all_records:
        raise RuntimeError("中古マンションデータを取得できませんでした。")
